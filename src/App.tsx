...
function App() {
  ...
  const [repoStatus, setRepoStatus] = useState(
    "Sign in to load repos or verify manual entries.",
  );
  const [previewUrl, setPreviewUrl] = useState("");
  const [releaseAppendStatus, setReleaseAppendStatus] =
    useState<ReleaseAppendStatus>("idle");
  ...
  useEffect(() => {
    getSession()
      .then((user) => {
        ...
        setRepoStatus(
          user
            ? "Load your GitHub repos or verify the manual repo."
            : "Sign in to load repos or verify manual entries.",
        );
      })
      .catch((error) =>
  ...
  async function handleCrop(crop: ImageCrop) {
    ...
    try {
      ...
      setStatus("idle");
      setMessage(
        `Compressed to ${formatBytes(nextCompressed.blob.size)} (${nextCompressed.width}×${nextCompressed.height}).`,
      );
    } catch (error) {
      setStatus("error");
      setMessage(
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("done");
      setMessage(
        "Done — copy this into your GitHub release notes or append it automatically.",
      );
    } catch (error) {
      setStatus("error");
    ...
    try {
      ...
      setReleaseAppendMessage(
        error instanceof Error
          ? error.message
          : "Could not update GitHub release notes.",
      );
    }
  }
  ...
  async function copyMarkdown(markdown: string) {
    try {
      ...
      setStatus("error");
      setMessage(
        "Could not copy Markdown. Select the snippet and copy it manually.",
      );
    }
  }
  ...
  async function loadRepos() {
    try {
      ...
      setRepoStatus(
        nextRepos.length
          ? "Choose a repo or keep typing manually."
          : "No accessible repos returned by GitHub.",
      );
    } catch (error) {
      setRepoStatus(
  ...
  async function handleRepoSelect(fullName: string) {
    ...
    try {
      ...
      setRepoStatus(
        nextReleases.length
          ? "Choose a release tag or type one manually."
          : "Repo verified. No releases found; type a tag manually.",
      );
    } catch (error) {
      setRepoStatus(
        error instanceof Error ? error.message : "Could not load releases.",
      );
    }
  }
  ...
  async function handleVerifyRepo() {
    try {
      ...
      setReleases(nextReleases);
      setRepoStatus(
        `Verified ${verified.fullName}. ${nextReleases.length ? "Pick a release below." : "No releases found; type a tag manually."}`,
      );
    } catch (error) {
      setRepoStatus(
        error instanceof Error ? error.message : "Could not verify repo.",
      );
    }
  }
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#fffaf2] px-3 py-5 font-sans text-neutral-800 transition-colors sm:px-4 sm:py-8 md:py-12 dark:bg-neutral-950 dark:text-neutral-100">
      <div className="mx-auto w-full max-w-6xl">
        <section className="grid min-h-0 items-center gap-5 sm:gap-8 lg:min-h-[56vh] lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            ...
            <div className="mt-6 grid gap-3 sm:mt-8 sm:flex sm:flex-wrap">
              <a href="#generator" className={primaryButtonClass}>
                Generate Markdown
              </a>
              <a href="#example" className={secondaryButtonClass}>
                See example
              </a>
            </div>
          </div>
        ...
        <section
          className="mt-6 grid gap-5 sm:mt-8 sm:gap-6 lg:grid-cols-[1.1fr_0.9fr]"
          id="generator"
        >
          <form
          ...
          >
            ...
            <div className="rounded-3xl border border-neutral-200 bg-neutral-50/80 p-4 dark:border-neutral-700 dark:bg-neutral-900/80">
              <div className="grid gap-3 sm:flex sm:flex-wrap sm:items-end">
                ...
                <CustomList
                  ...
                  getSubtitle={(item) =>
                    `${item.private ? "Private" : "Public"}${item.permission ? ` · ${item.permission}` : ""}`
                  }
                  getIcon={(item) =>
                    repoPrivateLabel[
                      String(item.private) as keyof typeof repoPrivateLabel
                    ]
                  }
                  placeholderTitle="Choose from GitHub..."
                  placeholderSubtitle={
                    repos.length
                      ? "Select a loaded repository"
                      : "Load repos to choose one"
                  }
                  placeholderIcon="📦"
            ...
            <p className={`min-h-6 ${statusClass}`}>
              {message ||
                "Images are cropped square and resized client-side to WebP under 50 KB."}
            </p>
            ...
            <div className="grid gap-2 sm:flex sm:flex-wrap">
              {result && (
                <button
                  ...
                  onClick={handleAppendToRelease}
                  type="button"
                  disabled={releaseAppendStatus === "updating"}
                >
                  {releaseAppendStatus === "updating"
                    ? "Updating release..."
                    : "Append to GitHub release"}
                </button>
              )}
              <button
                className={secondaryButtonClass}
